# This is legacy code that Ed originally shared with me for getting
# started on the iCGM sensitivity analysis figures. This file can probably be
# removed now.
...
if os.path.exists(demographic_datapath):
    ...
    file_reader = pd.read_csv(
        file_import_path + ".gz", low_memory=False, chunksize=10000
    )
    chunk_num = 1
    sim_data = []
...
if make_data_sample:
    ...
    bg_actual_sample.T.to_csv(
        os.path.join("..", "..", "data", "processed", "bg-array-sample.csv"),
        header=False,
        index=False,
    )

    bg_actual_sample = data_sample_all.loc[
        :, data_sample_all.columns.str.contains("bg_actual")
    ]
    bg_actual_sample.T.to_csv(
        os.path.join(
            "..",
            ...
            "data",
            "processed",
            "bg-matrix-sample-each-col-is-a-unique-time-series.csv",
        ),
        header=False,
        index=False,
    )
...
def get_summary_stats(df, level_of_analysis_name):
    ...
    df.rename(
        {
            "loop_LBGI": "LBGI",
            "loop_LBGI_RS": "LBGI Risk Score",
            "DKAI_RS": "DKAI Risk Score",
        },
        axis="columns",
        inplace=True,
    )
...
summary_df_reduced["DKAI Risk Score String"] = summary_df_reduced[
    "DKAI Risk Score"
].replace(score_dict)
summary_df_reduced["LBGI Risk Score String"] = summary_df_reduced[
    "LBGI Risk Score"
].replace(score_dict)
