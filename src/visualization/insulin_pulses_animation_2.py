...
#Create figure
def create_fig(file_location, filename):
    ...
    axes[2].grid(True)
    axes[2].set_title("Loop Decisions (scheduled basal rate and temp basals)")
    ...
    for t in sim_df["hours_post_simulation"]:
        ...
        axes[2].plot(sim_df_subset["hours_post_simulation"], sim_df_subset["temp_basal_sbr_if_nan"], color="#008ECC", drawstyle="steps-pre")
    ...
    loop_decision_handles.append(Line2D([0], [0], color="#008ECC", label="Loop Decision", linestyle="-", linewidth=1.5))
...
create_fig(insulin_pulse_file_location, filename)