...
def get_figure(df, x_col, y_col, selectedpoints, selectedpoints_local):
    ...
    fig.update_traces(selectedpoints=selectedpoints,
                      customdata=df.index,
                      mode='markers+text', marker={ 'color': 'rgba(0, 116, 217, 0.7)', 'size': 5}, unselected={'marker': {'color':'red', 'opacity': 0.3 }, 'textfont': { 'color': 'rgba(0, 0, 0, 0)' }})

    fig.update_layout(margin={'l': 20, 'r': 0, 'b': 15, 't': 5}, dragmode='select', hovermode='x')
...
def callback(selection1, selection2, selection3):
    ...
    return [get_figure(combined_df, "initial_bias_icgm", "LBGI Difference", selectedpoints, selection1),
            get_figure(combined_df, "noise_coefficient_icgm", "LBGI Difference", selectedpoints, selection2),
            get_figure(combined_df, "bias_drift_range_start_icgm", "LBGI Difference", selectedpoints, selection3)]
